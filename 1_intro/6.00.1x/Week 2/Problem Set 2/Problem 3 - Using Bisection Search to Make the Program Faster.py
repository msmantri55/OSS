...
minFixedMonthlyPayment = (upperBound + lowerBound) / 2
month = 1
...
while updatedBalance > 0:
    ...
    if updatedBalance < 0:
        if int(updatedBalance) != 0:
            # redefine bounds for search
            upperBound = minFixedMonthlyPayment
            minFixedMonthlyPayment = (upperBound + lowerBound) / 2

            # reset because minimum payment is too high
            month = 1
            updatedBalance = balance
        else:
            # print lowest payment if no balance remains
            print('Lowest Payment: %(payment).2f') % \
        ...
        # reset balance, increase payment by $0.01, and try again
        month = 1
        updatedBalance = balance
        minFixedMonthlyPayment += 0.01
